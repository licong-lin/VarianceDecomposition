...
def numerical_sobol(p,d,n,lam,kind='V_s',alpha=1,sig=0.3,k=20):
    ...
    for i in range(times):
        ...
        for j in range(k):
            if kind in ['mse','bias','var']:   
                ...
                record[j,i]=numerical_sobol(p0,d0,n,lam[i],kind,alpha,sig,20)
                theory=sobol_indices(pi,delt,kind,alpha,sig,lam)
...
def plot_numerical_bvm_sobol(sobol=False,filename='record'):  
...
##sobol=True: plot Sobol' indices (Figure 7 right).
##sobol=False:  plot Bias, Var and MSE (Figure 7 left).
#plot_numerical_bvm_sobol(sobol=True,filename='record')  ##filename='user_record'
